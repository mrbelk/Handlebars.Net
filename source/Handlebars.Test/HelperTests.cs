...
namespace HandlebarsDotNet.Test
{
    public class HelperTests
    {
        ...
        [Fact]
        public void BlockHelperWithEnumeration()
        {
            Handlebars.RegisterHelper ("ifCond", (writer, options, context, args) => {
                if (args.Length != 3) {
                    writer.Write ("ifCond:Wrong number of arguments");
            ...
            var template = Handlebars.Compile (@"{{#ifCond arg1.Count '>' 0}}{{#each arg1}}{{@this}},{{/each}}{{else}}nothing to enumerate{{/ifCond}}");
            var data = new { arg1 = new List<string> { "one", "two", "three" } };
            var result = template (data);
            Assert.Equal ("one,two,three,", result);

            data = new { arg1 = new List<string> () };
            result = template (data);
            Assert.Equal ("nothing to enumerate", result);
        }
